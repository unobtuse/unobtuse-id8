...
export default function ImageCropper({ file, onCrop, onCancel, aspectRatio = 1 }) {
  ...
  useEffect(() => {
    ...
    // Draw image
    ctx.drawImage(imageData.img, 0, 0, imageData.displayWidth, imageData.displayHeight);
  ...
  const handleMouseMove = (e) => {
    ...
    if (resizing) {
      const diff = Math.max(x - (cropArea.x + startPos.initialSize), y - (cropArea.y + startPos.initialSize));
      ...
      const newX = Math.max(0, Math.min(x - startPos.x, imageData.displayWidth - cropArea.size));
      const newY = Math.max(0, Math.min(y - startPos.y, imageData.displayHeight - cropArea.size));
    ...
    >
      <View style={[styles.container, { backgroundColor: colors.surface }]}>
        ...
        <View style={styles.buttons}>
          <TouchableOpacity 
            style={[styles.button, styles.cancelBtn, { borderColor: colors.glassBorder }]}
            onPress={onCancel}
          >
