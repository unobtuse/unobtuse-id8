...
const EMOJI_CATEGORIES = [
    { key: "recent", icon: "🕐", name: "Frequently Used" },
    { key: "Smileys & Emotion", icon: "😀", name: "Smileys & Emotion" },
    { key: "People & Body", icon: "👋", name: "People & Body" },
    { key: "Animals & Nature", icon: "🐶", name: "Animals & Nature" },
    { key: "Food & Drink", icon: "🍕", name: "Food & Drink" },
    { key: "Activities", icon: "⚽", name: "Activities" },
    { key: "Travel & Places", icon: "✈️", name: "Travel & Places" },
...
export default function MediaPicker({ visible, onSelectEmoji, onSelectSticker, onClose }) {
    ...
    const emojiSections = useMemo(() => {
        if (searchQuery.trim()) {
            ...
            return [{ title: "Search Results", key: "search", data: [allEmojis], isGrid: true }];
        }
    ...
    const renderBlurContainer = (children) => {
        ...
        return (
            <View style={[styles.container, { borderTopColor: colors.glassBorder }]}>
                ...
                <View style={[styles.blurOverlay, { backgroundColor: colors.glass }]}>
                    {children}
                </View>
            </View>
        );
        ...
        return renderBlurContainer(
            <>
                ...
                <View style={styles.namingContainer}>
                    ...
                    <TextInput
                        style={[styles.nameInput, { color: colors.text, borderColor: colors.glassBorder, backgroundColor: colors.surface }]}
                    ...
                    <View style={styles.namingButtons}>
                        <TouchableOpacity style={[styles.namingBtn, { borderColor: colors.glassBorder }]} onPress={cancelPendingSticker}>
                            <Text style={[styles.namingBtnText, { color: colors.text }]}>Cancel</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.namingBtn, styles.saveBtnPrimary, { backgroundColor: colors.accent }]} onPress={handleSaveSticker} disabled={uploading}>
                            {uploading ? <ActivityIndicator size="small" color="#000" /> : <Text style={[styles.namingBtnText, { color: "#000" }]}>Save</Text>}
                        </TouchableOpacity>
                    </View>
                </View>
            </>
    ...
    return renderBlurContainer(
        <>
            ...
            <View style={[styles.tabBar, { borderBottomColor: colors.glassBorder }]}>
                <TouchableOpacity
                    style={[styles.tab, activeTab === "emoji" && { borderBottomColor: colors.accent, borderBottomWidth: 2 }]}
                    onPress={() => setActiveTab("emoji")}
                >
                    <Text style={[styles.tabText, { color: activeTab === "emoji" ? colors.accent : colors.textSecondary }]}>😀 Emoji</Text>
                </TouchableOpacity>
                <TouchableOpacity
                    style={[styles.tab, activeTab === "sticker" && { borderBottomColor: colors.accent, borderBottomWidth: 2 }]}
                    onPress={() => setActiveTab("sticker")}
                >
                    <Text style={[styles.tabText, { color: activeTab === "sticker" ? colors.accent : colors.textSecondary }]}>🖼️ Stickers</Text>
                </TouchableOpacity>
                <View style={{ flex: 1 }} />
            ...
            {activeTab === "emoji" ? (
                <>
                    {/* Emoji Search */}
                    <View style={[styles.searchRow, { backgroundColor: colors.glass }]}>
                    ...
                    <View style={styles.contentContainer}>
                        {/* Emoji Sidebar */}
                        <View style={[styles.sidebar, { borderRightColor: colors.glassBorder, backgroundColor: colors.background }]}>
                            {EMOJI_CATEGORIES.map((cat) => (
                                <TouchableOpacity
                                    key={cat.key}
                                    style={[styles.sidebarBtn, activeCategory === cat.key && { backgroundColor: colors.accent + "20", borderLeftColor: colors.accent }]}
                            ...
                            renderItem={({ item }) => (
                                <View style={styles.emojiGrid}>
                                    {item.map((emoji, index) => (
                                        <TouchableOpacity key={`${emoji}-${index}`} style={styles.emojiBtn} onPress={() => handleSelectEmoji(emoji)}>
                ...
                <>
                    ...
                    {stickerLoading ? (
                        ...
                        <ScrollView style={styles.stickerGrid} contentContainerStyle={styles.stickerGridContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                            <View style={styles.stickerRow}>
                                {filteredStickers.map((sticker) => (
                                    <TouchableOpacity key={sticker.id} onPress={() => onSelectSticker(sticker)} style={styles.stickerItem}>
                                        {isVideo(sticker.file_type) ? (
                                            <View style={[styles.stickerMedia, styles.videoPlaceholder]}>
                                                <Ionicons name="videocam" size={24} color="#fff" />
                                            </View>
                                        ) : (
                                            <Image source={{ uri: sticker.file_url }} style={styles.stickerMedia} resizeMode="contain" />
                                        )}
                                        <Text style={[styles.stickerName, { color: colors.textTertiary }]} numberOfLines={1}>:{sticker.name}:</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                        </ScrollView>
            ...
            {Platform.OS === 'web' && (
                <input type="file" ref={stickerInputRef} style={{ display: 'none' }} accept="image/*,video/*,.gif" onChange={handleWebChange} />
            )}
        </>
    );
}
