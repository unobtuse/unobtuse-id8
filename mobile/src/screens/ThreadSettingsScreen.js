...
export default function ThreadSettingsScreen({ route, navigation }) {
  ...
  const fetchData = async () => {
    try {
      const [ideaData, collabData, notifData] = await Promise.all([
        ...
        api
          .get(`/collaborators/idea/${ideaId}/notifications`)
          .catch(() => ({ notifications_enabled: true })),
      ]);
      setIdea(ideaData);
  ...
  const toggleNotifications = async (enabled) => {
    ...
    try {
      await api.patch(`/collaborators/idea/${ideaId}/notifications`, {
        enabled,
      });
    } catch (error) {
  ...
  const canChangeBg =
    isOwner ||
    collaborators.find(
      (c) => c.user_id === user?.id && c.can_change_background,
    );

  const toggleMemberInvites = async (value) => {
    try {
      await api.patch(`/ideas/${ideaId}/settings`, {
  ...
  const toggleCanInvite = async (collabId, value) => {
    try {
      await api.patch(`/collaborators/${collabId}/permissions`, {
        can_invite: value,
      });
  ...
  const toggleCanChangeBg = async (collabId, value) => {
    try {
      await api.patch(`/collaborators/${collabId}/permissions`, {
        can_change_background: value,
      });
      setCollaborators((prev) =>
        prev.map((c) =>
          c.id === collabId ? { ...c, can_change_background: value } : c,
        ),
      );
    } catch (error) {
      console.error("Failed to update permissions:", error);
                ...
                <View style={styles.bgSection}>
                  ...
                  >
                    {uploadingBg ? (
                      ...
                      <Image
                        source={{ uri: idea.background_url }}
      ...
      {iosModal && Platform.OS === "web" && (
        ...
        >
          ...
          >
            ...
            >
              To receive push notifications on iPhone, you need to add ID8 to
              your home screen.
            </Text>
            <View
              style={{
            ...
            >
              ...
              >
                <Text style={{ color: colors.text, fontSize: 14 }}>
                  <Text style={{ fontWeight: "600" }}>1.</Text> Tap the
                  <Text style={{ fontWeight: "600" }}>Share</Text> button
                </Text>
                <Ionicons
                  name="share-outline"
                  size={16}
              ...
              <Text style={{ color: colors.text, fontSize: 14 }}>
                <Text style={{ fontWeight: "600" }}>2.</Text> Scroll down and
                tap
